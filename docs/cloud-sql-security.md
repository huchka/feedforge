...
## SSL Enforcement
Direct connections to the Cloud SQL instance require SSL (`require_ssl = true`). This prevents any unencrypted direct connections from reaching the database. Note that the Cloud SQL Auth Proxy sidecar automatically handles the encrypted tunnel when connecting via Workload Identity.
...
- **Username Substring**: Passwords cannot contain the username
